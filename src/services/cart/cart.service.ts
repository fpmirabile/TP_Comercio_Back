...
export const getCartItemsByProductAndCart = async (cartId: string, productId: string): Promise<CartItem | undefined> => {
  const cartRepository = getRepository(CartItem);
  const cartItems = await cartRepository.findOne({ where: { cart: { id: cartId }, product: { id: productId } }, relations: ['cart', 'product'] });
  return cartItems;
}

export const getCartItemsByCart = async (cartId: string): Promise<CartItem[]> => {
  const cartRepository = getRepository(CartItem);
  const cartItems = await cartRepository.find({ where: { cart: { id: cartId } }, relations: ['cart', 'product'] });
  return cartItems;
}
...
export const getUserCart = async (userId: string): Promise<Cart> => {
  const cartRepository = getRepository(Cart);
  const user = await getUserById(userId);
  let cart = await cartRepository.findOne({ where: { user }, relations: ['items', 'items.product'] });
  if (!cart) {
...
export const deleteItemFromCart = async (userId: string, prodId: string): Promise<boolean> => {
  ...
  const delOperation = cartItemRepository.delete({ cart: { id: cart.id }, product: { id: prodId } })
  return !!(await delOperation).affected;
}

export const deleteAllProductsFromCart = async (cartId: string): Promise<boolean> => {
  const cartItemRepository = getRepository(CartItem);
  const cartItems = await getCartItemsByCart(cartId);
  const prodIds = cartItems.map(i => { return i.product.id });
  const operation = await cartItemRepository.delete({ product: { id: In(prodIds) }, cart: { id: cartId } });
